...
full_state_response::full_state_response(std::string game_id, Game state) :
...
full_state_response *full_state_response::from_json(const rapidjson::Value& json) {
    if (json.HasMember("state_json")) {
        ...
        throw ZombieDiceException("Could not parse full_state_response from json. state is missing.");
    }
}
