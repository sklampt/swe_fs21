...
std::string client_join_lobby_request::undefined_game_id {""};

// Public constructor
client_join_lobby_request::client_join_lobby_request(std::string player_id, std::string name)
        : client_request( client_request::create_base_class_properties(RequestType::join_game, uuid_generator::generate_uuid_v4(), player_id, client_join_lobby_request::undefined_game_id) ),
          _player_name(name)
{ }

client_join_lobby_request::client_join_lobby_request(std::string game_id, std::string player_id, std::string name)
...
request_response* client_join_lobby_request::execute() {
    Player* player;
    ...
    if (_game_id.empty()) {
        ...
        if (game_instance_manager::try_add_player_to_any_game(player, game_instance_ptr, err)) {
            ...
            return new request_response(game_instance_ptr->get_id(), _req_id, true,
                                        game_instance_ptr->get_game()->to_json(), err);
        } else {
        ...
        if (game_instance_manager::try_get_game_instance(_game_id, game_instance_ptr)) {
            if (game_instance_manager::try_add_player(player, game_instance_ptr, err)) {
                // return response with full game_state attached
                return new request_response(_game_id, _req_id, true, game_instance_ptr->get_game()->to_json(), err);
            } else {
                return new request_response("", _req_id, false, nullptr, err);
            }
        } else {
